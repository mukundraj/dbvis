...
using namespace HalfspaceDepth;

extern "C" double our_function(int N, int d, double *xs);
double our_function(int N, int d, double *xs) {
//    int i;
//    int sum;
...
//    }
      ...
      function<double(double*, double**, int, int)> HD_Alg[] = { HD_Rec, HD_Comb, HD_Comb2 };
  const string AlgName[] = { "HD_Rec", "HD_Comb", "HD_Comb2" };

  mt19937 gen(1);
  uniform_int_distribution<int> uniform_int(-2, 2);
  normal_distribution<double> normal;
  function<double()> rnd_dist[] = { bind(normal, gen), bind(uniform_int, gen) };
  const string DistName[] = { "standard normal", "uniform(-2,-1,0,1,2)" };
  ...
  function<double()> rnd = rnd_dist[nDistribution];
  function<double(double*, double**, int, int)> HD = HD_Alg[nAlgorithm];

  x = new double*[nPunkte];
  z = new double[dim];
  for (int i = 0; i < nPunkte; i++) x[i] = new double[dim];
  for (int j = 0; j < dim; j++) z[j] = 0;
//    cout<<dim<<endl;
    for (int i = 0; i < nPunkte; i++){
        for (int j = 0; j < dim; j++){
           x[i][j] = xs[i+j*nPunkte];
//           x[i][j] = xs[i+j*nPunkte];
//            cout<<xs[i]<<endl
//            cout<<x[i][j]<<",";
