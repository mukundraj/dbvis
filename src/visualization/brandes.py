...
Radial constrained mds for graph layout. The Brandes paper version for comparison.
...
class rmds(object):
    ...
    def __init__(self, depths, N_i, M_i,res,bw, max_step, num_old, num_old_spline, depth_type, alpha=None, adj_mat=None):
        ...
        self.rs = (1-self.depths)*0.5+0.000001
        self.inv_rs = np.reciprocal(self.rs)
        self.inv_rs = self.inv_rs * self.inv_rs
    ...
    def fit_linear_fn(self, xs,ys,res=100):
        ...
        sigma =np.ones(N)
        sigma[[0]] = 0.01
        p, _ = optimize.curve_fit(f, xs, ys, (0, 0), sigma=sigma)
        x2 = np.linspace(min(xs), max(xs), res)
        y2 = f(x2, *p)
        return x2,y2
    ...
    def compute_depth_spline(self,cur_pos, iter=None):
        ...
        # dprint(ZI_pol_mono[0,:])
        # ZI_pol_mono[0:5,:]=0
        # ZI_pol_mono[:,10:20]=0
        # dprint(np.amax(ZI_pol_mono),np.amin(ZI_pol_mono))

        #
        # for j in range(self.res):
        #     col = ZI_pol_mono[:,j]
        #     # plt.plot(RI[:, j], col, "r")
        #     x2,y2 = self.fit_linear_fn(RI[:, j], col, res=self.res)
        #     ZI_pol_mono[:,j] = y2
    ...
    def get_total_forces(self, mds_dist_mat, cur_dist_mat, cur_pos, XI2, YI2, ZIcm,fixed_stepsize, depth_factor, smooth_factor, itr):
        ...
        W2 = np.zeros((N+1,N+1))
        W2[N,:N] = self.inv_rs.reshape((1,N))
        W2[:N,N] = self.inv_rs.reshape((1,N))
        ...
        if (iter > 300): #700 for synthetic talk example
            W3 = W2
        else:
            W3 = (1-lamda)*W + lamda*W2

        cur_pos_with_origin = copy.deepcopy(cur_pos)
        cur_pos_with_origin.append(np.array([0,0]))
        # dprint(cur_pos_with_origin)
        cur_dist_mat = self.list_to_dist_mat(cur_pos_with_origin)
        ...
        B = self.get_B_matrix(N+1,D, cur_dist_mat, W3)
        V = self.get_V_matrix(D, W3)
        grad_sig = 2*(np.dot(V,Z)-np.dot(B,Z))
        # grad_sig /= 2*N
        ...
        gd_total = grad_sig + smooth_factor*grad_sm
        ...
        # return the step
        tfs = alpha*gd_total
        return tfs, grad_sm, grad_sig, grad_sm
    ...
    def get_mds_dc(self, original_points, cur_pos, show_fig, spline_lag, depth_factor, smooth_factor, orig_dist_mat=None,fixed_stepsize=None, names = None):
        ...
        self.W = np.reciprocal(self.original_dist_mat)
        ...
        dis_energy = np.sum((((self.original_dist_mat-cur_dist_mat)*self.W)/2)**2)
        ...
        for iter in range(self.N_i):
            ...
            tfs,dfgd,dgd, sgd = self.get_total_forces(self.original_dist_mat, None,cur_pos,XI2,YI2, ZIcm,fixed_stepsize, depth_factor, smooth_factor, iter)
