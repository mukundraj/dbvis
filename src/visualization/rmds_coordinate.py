...
class rmds(object):
    ...
    def total_linesearch(self, ZIcm, XI2, YI2, gd, cur_pos, rho, c):
        ...
        # store orig x
        xx = np.array(cur_pos)
        fk = self.get_total_energy(ZIcm, XI2, YI2, xx)
        x = copy.deepcopy(xx)
        ...
        fk1 = self.get_total_energy(ZIcm, XI2, YI2, x)
    ...
    def get_total_forces(self, mds_dist_mat, cur_dist_mat, cur_pos, XI2, YI2, ZIcm,fixed_stepsize, depth_factor, smooth_factor, iter):
        ...
        gs = self.A*cur_pos
        grad_sm = np.array(gs)
        gd_total = grad_sig + depth_factor*grad_dep+ smooth_factor*grad_sm

        cur_pos_copy = copy.deepcopy(cur_pos)

        tfs = np.zeros_like(grad_sig)
        cur_grad = np.zeros_like(gd_total)
        for j in range(N):

            # isolate the jth gradient
            cur_grad[j-1,:] = 0
            cur_grad[j,:] = gd_total[j,:]

            # find step size for jth point using cur_pos_copy
            alpha = self.total_linesearch(ZIcm, XI2, YI2, cur_grad, cur_pos_copy, rho=0.5, c=0.0001)
            dprint(j,alpha)



            # update cur_pos_copy and tfs
            delta = alpha*cur_grad
            tfs = tfs + delta
            # dprint(delta)
            # dprint(tfs)
            # dprint(cur_pos_copy)
            cur_pos_copy[j] = cur_pos_copy[j] + delta[j,:]
