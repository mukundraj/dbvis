...
General version of ARL(started after the GD17 submissiont).
Similar to ARL, cleaner code and without the graph smoothing.
...
class garl(object):
    ...
    def list_to_dist_mat(self, pos):
        ...
        dprint("###ALERT still checking for negative dists ####")
    ...
    def compute_depth_spline(self,cur_pos, iter=None):
        ...
        # tps = Rbf(xs,ys,self.depths,function='linear')
        ...
        ##for presentation figure on monotonization, code starts
        # ZIcm = tps(XI2,YI2)
    ...
    def get_total_forces(self, mds_dist_mat, cur_dist_mat, cur_pos, XI2, YI2, ZIcm,fixed_stepsize, depth_factor, smooth_factor, itr):
        ...
        gd_total = lamda*grad_sig + depth_factor*grad_dep
    ...
    def get_mds_dc(self, original_points, cur_pos, show_fig, spline_lag, depth_factor, smooth_factor, orig_dist_mat=None,fixed_stepsize=None, names = None):
        ...
        orig_one_inds = np.where(self.depths==1)
        hs_for_ones = depths_curpos[orig_one_inds]
        self.ori_median_ind = orig_one_inds[0][np.argmax(hs_for_ones)]
        ...
        cur_pos,XI2,YI2,ZIcm = self.compute_depth_spline(cur_pos)
        self.old_ZIcm = ZIcm
        ...
        if original_points is not None:
            ...
            # dprint(np.count_nonzero(np.isinf(np.reciprocal(self.original_dist_mat))))
            # exit(0)
        ...
        if self.alpha is not None:
            ...
            band_levels = [band_prob_bounds_pre[1]['lower'], band_prob_bounds_pre[1]['higher']]
        ...
        data_dict['median1_ind'] = self.ori_median_ind
        data_dict['band_levels'] = band_levels
        ...
        for iter in range(self.N_i):
            ...
            if iter%spline_lag==0:
                cur_pos,XI2,YI2,ZIcm = self.compute_depth_spline(cur_pos, iter)
                ZIcm = (ZIcm + self.old_ZIcm)/2
                self.old_ZIcm = ZIcm

                self.adaptive_enabled = False

            tfs,dfgd,dgd = self.get_total_forces(self.original_dist_mat, cur_dist_mat,cur_pos,XI2,YI2, ZIcm,fixed_stepsize, depth_factor, smooth_factor, iter)
            ...
            if iter%spline_lag==0:
                ...
                ZIcm_linear = np.ndarray.flatten(np.flipud(ZIcm))
                data_dict = {}
                data_dict["field"] = np.around(ZIcm_linear, decimals=3).tolist()
                data_dict['maxx'] = round(XI2[-1,-1],3)
                data_dict['minx'] = round(XI2[0,0],3)
                data_dict['maxy'] = round(YI2[-1,-1],3)
                data_dict['miny'] = round(YI2[0,0],3)
                ...
                if self.alpha is not None and iter==self.N_i-1:
                    fig = plt.figure(frameon=False)
                    fig.set_size_inches(w=4,h=4)
                    ax = plt.Axes(fig, [0., 0., 1., 1.])
                    ax.set_axis_off()
                    ...
                    CS3 = plt.contourf(XI2, YI2, ZIcm, band_levels, colors=("#ffffff","#00ccff", '#0000ff'), extend='both')
                    CS4 = plt.contour(XI2, YI2, ZIcm, band_levels, colors=('k',),linewidths=(1,), linestyles='solid', antianliasted=True)
                    plt.axis('off')

                    imname ='output_tsvs/bag_'+format(int(math.floor(iter/spline_lag)), '04')+'.png'
                    fig.savefig(imname, dpi=100)
                    im = Image.open(imname)
                    im.putalpha(128)
                    im.save(imname)
            ...
            if iter%spline_lag==0:
                ...
                plt.plot(range(2+(iter)),forces[0],'g--',label="Depth Penalty")
                plt.plot(range(2+(iter)),total_energy,'b--',label="Total Energy")
                ...
                # plt.title('Total energy')
                # # plt.plot(range(2+(iter)),forces[1],'r--',label="MDS stress")
                # lns1 = ax.plot(range(2+(iter)),forces[0],'g--',label="Depth penalty")
                ...
                #
                # ax2 = ax.twinx()
                # lns3 = ax2.plot(range(2+(iter)),forces[1],'r--',label="")
                # ax2.set_ylim([0.00,0.2])
                # ax2.set_ylabel('MDS stress', fontsize=14)
                # plt.grid(True)
                # plt.legend(loc=1,prop={'size':14})
                #
