...
class rmds_bi(object):
    """

    """

    def __init__(self, depths1, depths2, c1inds, c2inds, N_i, M_i,res,bw, max_step, num_old, num_old_spline, depth_type, alpha=None, adj_mat=None):
        """
        ...
        # self.depths_max1 = np.amax(self.depths1)
        # self.depths_min1 = np.amin(self.depths1)
        # self.depths_range1 = self.depths_max1 - self.depths_min1
        #
        # self.depths_max2 = np.amax(self.depths2)
        # self.depths_min2 = np.amin(self.depths2)
        # self.depths_range2 = self.depths_max2 - self.depths_min2
    ...
    def get_depth_engergy(self,  ZIcm, XI2, YI2, cur_pos, depth_factor, depths):
        """Get the curre
        ...
        depth_energy = depth_factor*np.sum((depths - depths_curpos)**2)
    ...
    def compute_depth_spline(self,cur_pos, depths, iter=None):
    ...
    def get_total_forces(self, mds_dist_mat, cur_dist_mat, cur_pos, XI2, YI2, ZIcm,fixed_stepsize, depth_factor, smooth_factor, itr):
        ...
        # gs = []
        # for i in range(N):
        #     ix = cur_ids[i][1];iy = cur_ids[i][0]
        #     gx = 0
        #     gy = 0
        #     for j in range(N):
        #         jx = cur_ids[j][1];jy = cur_ids[j][0]
        #         gx += self.A[i,j]*(ix-jx)
        #         gy += self.A[i,j]*(iy-jy)
        #
    ...
    def get_mds_dc(self, original_points, cur_pos, show_fig, spline_lag, depth_factor, smooth_factor, orig_dist_mat=None,fixed_stepsize=None, names = None):
        ...
        energies = [[],[],[],[]]
        dprint(cur_pos)
        cur_pos1 = [cur_pos[i] for i in self.c1inds]
        cur_pos2 = [cur_pos[i] for i in self.c2inds]
        ...
        cur_pos1,XI2_a,YI2_a,ZIcm_a, depths_curpos_a = self.compute_depth_spline(cur_pos1, self.depths1)
        self.old_ZIcm_a = ZIcm_a
        depth_energy1 = self.get_depth_engergy(ZIcm_a, XI2_a, YI2_a, cur_pos1, depth_factor, self.depths1)
        depth_energy1 /= N
        energies[0].append(depth_energy1)

        cur_pos2,XI2_b,YI2_b,ZIcm_b, depths_curpos_b = self.compute_depth_spline(cur_pos2, self.depths2)
        self.old_ZIcm_a = ZIcm_a
        depth_energy2 = self.get_depth_engergy(ZIcm_a, XI2_a, YI2_a, cur_pos2, depth_factor, self.depths2)
        depth_energy2 /= N
        energies[0].append(depth_energy2)
        ...
        for iter in range(self.N_i):
            ...
            if iter%spline_lag==0:
                ...
                fig.add_axes(ax)
                plt.axis('equal')
                plt.pcolor(XI2, YI2, ZIcm, cmap=cm.autumn_r, vmin=ZIcm_min, vmax=ZIcm_max)
